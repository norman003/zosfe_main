*---------------------------------------------------------------------*
*    program for:   VIEWFRAME_ZOSFEVA_UBL
*---------------------------------------------------------------------*
FUNCTION VIEWFRAME_ZOSFEVA_UBL         .
...
*----------------------------------------------------------------------*
CALL FUNCTION 'VIEWPROC_ZOSFEVA_UBL'
         EXPORTING
              FCODE          = READ
...
EXTRACT = ZOSFEVA_UBL_EXTRACT
TOTAL = ZOSFEVA_UBL_TOTAL
              X_HEADER       = X_HEADER
              X_NAMTAB       = X_NAMTAB
...
CALL FUNCTION 'VIEWPROC_ZOSFEVA_UBL'
             ...
             IMPORTING
                  UCOMM           = FUNCTION
UPDATE_REQUIRED = STATUS_ZOSFEVA_UBL-UPD_FLAG
             TABLES
                  EXCL_CUA_FUNCT  = EXCL_CUA_FUNCT
EXTRACT = ZOSFEVA_UBL_EXTRACT
TOTAL = ZOSFEVA_UBL_TOTAL
...
*----------------------------------------------------------------------*
      ...
      IF ( FUNCTION EQ SWITCH_TO_SHOW_MODE OR
           ...
           FUNCTION EQ END ) AND
STATUS_ZOSFEVA_UBL-UPD_FLAG NE SPACE.
        PERFORM BEENDEN.
        CASE SY-SUBRC.
          WHEN 0.
CALL FUNCTION 'VIEWPROC_ZOSFEVA_UBL'
...
TOTAL = ZOSFEVA_UBL_TOTAL
            ...
            CASE SY-SUBRC.
              WHEN 0.
IF STATUS_ZOSFEVA_UBL-UPD_FLAG EQ SPACE. EXIT. ENDIF.
...
IF STATUS_ZOSFEVA_UBL-UPD_FLAG NE SPACE.
