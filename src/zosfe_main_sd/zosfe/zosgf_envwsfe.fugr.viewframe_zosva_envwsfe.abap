*---------------------------------------------------------------------*
*    program for:   VIEWFRAME_ZOSVA_ENVWSFE
*---------------------------------------------------------------------*
FUNCTION VIEWFRAME_ZOSVA_ENVWSFE       .
...
*----------------------------------------------------------------------*
CALL FUNCTION 'VIEWPROC_ZOSVA_ENVWSFE'
         EXPORTING
              FCODE          = READ
...
EXTRACT = ZOSVA_ENVWSFE_EXTRACT
TOTAL = ZOSVA_ENVWSFE_TOTAL
              X_HEADER       = X_HEADER
              X_NAMTAB       = X_NAMTAB
...
CALL FUNCTION 'VIEWPROC_ZOSVA_ENVWSFE'
             ...
             IMPORTING
                  UCOMM           = FUNCTION
UPDATE_REQUIRED = STATUS_ZOSVA_ENVWSFE-UPD_FLAG
             TABLES
                  EXCL_CUA_FUNCT  = EXCL_CUA_FUNCT
EXTRACT = ZOSVA_ENVWSFE_EXTRACT
TOTAL = ZOSVA_ENVWSFE_TOTAL
...
*----------------------------------------------------------------------*
      ...
      IF ( FUNCTION EQ SWITCH_TO_SHOW_MODE OR
           ...
           FUNCTION EQ END ) AND
STATUS_ZOSVA_ENVWSFE-UPD_FLAG NE SPACE.
        PERFORM BEENDEN.
        CASE SY-SUBRC.
          WHEN 0.
CALL FUNCTION 'VIEWPROC_ZOSVA_ENVWSFE'
...
TOTAL = ZOSVA_ENVWSFE_TOTAL
            ...
            CASE SY-SUBRC.
              WHEN 0.
IF STATUS_ZOSVA_ENVWSFE-UPD_FLAG EQ SPACE. EXIT. ENDIF.
...
IF STATUS_ZOSVA_ENVWSFE-UPD_FLAG NE SPACE.
