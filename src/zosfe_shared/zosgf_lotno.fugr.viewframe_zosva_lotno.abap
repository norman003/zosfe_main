*---------------------------------------------------------------------*
*    program for:   VIEWFRAME_ZOSVA_LOTNO
*---------------------------------------------------------------------*
FUNCTION VIEWFRAME_ZOSVA_LOTNO         .
...
*----------------------------------------------------------------------*
CALL FUNCTION 'VIEWPROC_ZOSVA_LOTNO'
         EXPORTING
              FCODE          = READ
...
EXTRACT = ZOSVA_LOTNO_EXTRACT
TOTAL = ZOSVA_LOTNO_TOTAL
              X_HEADER       = X_HEADER
              X_NAMTAB       = X_NAMTAB
...
CALL FUNCTION 'VIEWPROC_ZOSVA_LOTNO'
             ...
             IMPORTING
                  UCOMM           = FUNCTION
UPDATE_REQUIRED = STATUS_ZOSVA_LOTNO-UPD_FLAG
             TABLES
                  EXCL_CUA_FUNCT  = EXCL_CUA_FUNCT
EXTRACT = ZOSVA_LOTNO_EXTRACT
TOTAL = ZOSVA_LOTNO_TOTAL
...
*----------------------------------------------------------------------*
      ...
      IF ( FUNCTION EQ SWITCH_TO_SHOW_MODE OR
           ...
           FUNCTION EQ END ) AND
STATUS_ZOSVA_LOTNO-UPD_FLAG NE SPACE.
        PERFORM BEENDEN.
        CASE SY-SUBRC.
          WHEN 0.
CALL FUNCTION 'VIEWPROC_ZOSVA_LOTNO'
...
TOTAL = ZOSVA_LOTNO_TOTAL
            ...
            CASE SY-SUBRC.
              WHEN 0.
IF STATUS_ZOSVA_LOTNO-UPD_FLAG EQ SPACE. EXIT. ENDIF.
...
IF STATUS_ZOSVA_LOTNO-UPD_FLAG NE SPACE.
